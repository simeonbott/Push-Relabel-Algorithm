...
with open('bn-mouse-kasthuri_graph_v4.edges',"r") as f:
    edges_list = [line.split(" ") for line in f.read().split("\n")]
    s = [int(element[0]) for element in edges_list[:-1]]
    t = [int(element[1]) for element in edges_list[:-1]]


source = [0]*6 + [150] + [0]*1022 #input value - of length the number of nodes (including zero)
sink = [1]*1029 #input value - of length the number of nodes (including zero)
edge_capacity = [1]*1700 #input value - of length the number of edges
...
def PushRelabel(limit,number_nodes):
    ...
    if Queue:
        ...
        node = Queue[0][0]
        rounde = 0
        while Queue and rounde < 1000000:
            rounde += 1
